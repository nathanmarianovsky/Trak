...
*/
exports.showAdd = (BrowserWindow, path, fs, log, https, tools, mainWindow, dataPath, evnt, data, auto) => {
    ...
    if(!fs.existsSync(path.join(dataPath, "Trak", "data", data[0] + "-" + tools.formatFolderName(data[1])))) {
        ...
        tools.writeDataFile(log, mainWindow, BrowserWindow.getFocusedWindow(), exports.showObjCreation(path, fs, https, tools, dataPath, data), "A", dataPath, fs, path, evnt, data, false);
    }
    else {
...
*/
exports.showUpdate = (BrowserWindow, path, fs, log, https, tools, mainWindow, dataPath, evnt, data, auto) => {
    ...
    if(data[1] != "" && data[1] != data[data.length - 1]) {
        fs.rename(path.join(dataPath, "Trak", "data", data[0] + "-" + tools.formatFolderName(data[data.length - 1])), path.join(dataPath, "Trak", "data", data[0] + "-" + tools.formatFolderName(data[1])), err => {
            ...
            // If no error occured in renaming the record folder write the data file, and copy over the file assets.
                ...
                tools.writeDataFile(log, mainWindow, BrowserWindow.getFocusedWindow(), exports.showObjCreation(path, fs, https, tools, dataPath, data), "U", dataPath, fs, path, evnt, data, auto);
            }
        });
    }
    else {
