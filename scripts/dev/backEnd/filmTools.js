    ...
    if(data[1] != "" && data[1] != data[data.length - 1]) {
        ...
        if(q == compareList.length) {
            ...
            fs.rename(path.join(dataPath, "Trak", "data", fldrValue), path.join(dataPath, "Trak", "data", newFldr), err => {
                ...
                if(err) {
                    log.error("There was an issue in renaming the film record folder " + data[data.length - 1] + " to " + data[1] + ". Error Type: " + err.name + ". Error Message: " + err.message + ".");
...
exports.filmSearch = (log, movier, ev, search) => {
    ...
    }).catch(err => log.error("There was an issue in obtaining the film search results for autocomplete options associated to the query " + search[1] + ". Error Type: " + err.name + ". Error Message: " + err.message + "."));
};
...
exports.filmFetchDetails = (log, movier, tools, ev, name) => {
    ...
    }).catch(err => log.error("There was an issue in obtaining the details associated to the film name " + name + ". Error Type: " + err.name + ". Error Message: " + err.message + "."));
};
...
exports.filmFetchSearch = (log, movier, ev, search, path) => {
    if(search[1] == 1) {
        ...
        movier.searchTitleByName(search[0]).then(results => {
            ...
            const detPromise = new Promise((resolve, reject) => {
                ...
                results.forEach(elem => {
                    ...
                    movier.getTitleDetailsByUrl(elem.url).then(elemData => {
                        ...
                        log.error("There was an issue getting the film details based on the url " + elem.url + ". Error Type: " + err.name + ". Error Message: " + err.message + ".");
            ...
            }).catch(err => log.error("There was an issue resolving the promise associated to grabbing film details based on the search query " + search[0] + ". Error Type: " + err.name + ". Error Message: " + err.message + "."));
        }).catch(err => {
...
exports.filmSynopsisFetch = (log, movier, ev, link) => {
    ...
    }).catch(err => log.error("There was an issue getting the film synopsis based on the url " + link + ". Error Type: " + err.name + ". Error Message: " + err.message + "."));
};
...
exports.filmRecordRequest = (BrowserWindow, ipc, path, fs, log, https, movier, tools, globalWin, win, usrDataPath, link) => {
    ...
    }).catch(err => log.error("There was an issue getting the film details based on the url " + link + ". Error Type: " + err.name + ". Error Message: " + err.message + "."));
};
