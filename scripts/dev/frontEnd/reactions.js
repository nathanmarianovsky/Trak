...
BASIC DETAILS: This file handles all reactions on the primary page.
...
ipcRenderer.on("loadRows", event => {
    // // Define the html links for the menu items and the associated callbacks for a click event.
    // const home = document.getElementById("homeLink"),
    //     homeDiv = document.getElementById("homeDiv"),
    //     scheduler = document.getElementById("schedulerLink"),
    //     schedulerDiv = document.getElementById("schedulerDiv"),
    //     form = document.getElementById("formLink"),
    //     formDiv = document.getElementById("formDiv"),
    //     vehicleDetails = document.getElementById("vehicleDetailsLink"),
    //     vehicleDetailsDiv = document.getElementById("vehicleDetailsDiv"),
    //     fuel = document.getElementById("fuelLink"),
    //     fuelDiv = document.getElementById("fuelDiv"),
    //     timesheets = document.getElementById("timesheetsLink"),
    //     timesheetsDiv = document.getElementById("timesheetsDiv"),
    //     contacts = document.getElementById("contactsLink"),
    //     contactsDiv = document.getElementById("contactsDiv");
    // let homeFunc = e => { e.preventDefault(); ipcRenderer.send("homeLoad"); },
    //     schedulerFunc = e => { e.preventDefault(); ipcRenderer.send("scheduleLoad"); },
    //     formFunc = e => { e.preventDefault(); ipcRenderer.send("formLoad"); },
    //     vehicleDetailsFunc = e => { e.preventDefault(); ipcRenderer.send("vehicleDetailsLoad"); },
    //     fuelFunc = e => { e.preventDefault(); ipcRenderer.send("fuelLoad"); },
    //     timesheetsFunc = e => { e.preventDefault(); ipcRenderer.send("timesheetsLoad"); },
    //     contactsFunc = e => { e.preventDefault(); ipcRenderer.send("contactsLoad"); };
    // // Listen for a click event on a menu item.
    // home.addEventListener("click", homeFunc);
    // scheduler.addEventListener("click", schedulerFunc);
    // form.addEventListener("click", formFunc);
    // vehicleDetails.addEventListener("click", vehicleDetailsFunc);
    // fuel.addEventListener("click", fuelFunc);
    // timesheets.addEventListener("click", timesheetsFunc);
    // contacts.addEventListener("click", contactsFunc);
    // // The associated menu item should not be clickable.
    // contacts.removeEventListener("click", contactsFunc);

    // Define the path for all items and get a list of all available records.
    const pathDir = path.join(localPath, "Trak", "data", "records.json");
    let list = [];
    ...
    contactsTableBody.textContent = "";
    document.getElementById("contactsPreloader").style.setProperty("display", "none", "important");
