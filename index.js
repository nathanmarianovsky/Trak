...
BASIC DETAILS: This file handles the initialization of the application.

   - errHandle: Processes the throwing of an error by reading the logs file and providing the front end with it.
   - passProcess: Processes the application load after a password check, if necessary.
...
Processes the application load after a password check, if necessary.

   - openingWindow is an object corresponding to the splash window created by the application.

*/
const passProcess = openingWindow => {
				...
				if(localPagesDirErr) {
					...
					fs.mkdir(localStylesDir, { "recursive": true }, localStylesDirErr => {
						...
						if(localStylesDirErr) {
							...
							openingWindow.webContents.send("loadBlink", 3);
																				...
																				if(readErr) {
																					...
																					fs.writeFile(path.join(basePath, "Trak", "localStyles", "styles.css"), stylesFile, "UTF8", writeErr => {
																						...
																						if(writeErr) {
																							...
																							openingWindow.webContents.send("loadBlink", 4);
																							...
																							primaryWindow.webContents.on("did-finish-load", () => {
																								...
																								if(splashCheck == false) {
																									openingWindow.webContents.send("loadBlink", 5);
																									splashCheck = true;
																								}
																							...
																							require("./scripts/dist/backEnd/appListeners").addListeners(app, shell, BrowserWindow, path, fs, log, crypto, dev, ipc, tools, categoryActiveArr, goodreadsScraper, malScraper, movier,
																								updateCheck, primaryWindow, openingWindow, localPath, basePath, primWinWidth, primWinHeight, primWinFullscreen, secWinWidth, secWinHeight, secWinFullscreen);
																						}
																					});
																				}
																			});
...
app.whenReady().then(() => {
	...
	ipc.on("closeSplash", event => splashWindow.close());
	splashWindow.webContents.on("did-finish-load", () => {
		...
		fs.mkdir(assetsDir, { "recursive": true }, assetsDirErr => {
			...
			// Otherwise proceed normally.
				...
				const profilePath = path.join(basePath, "Trak", "config", "profile.json");
				// If a profile configuration file does not exist create one and proceed loading the application.
				if(!fs.existsSync(profilePath)) {
					fs.writeFileSync(profilePath, JSON.stringify({"pass": ""}), "UTF8");
					passProcess(splashWindow);
				}
				// Otherwise if a profile configuration file exists then read it.
				else {
					fs.readFile(profilePath, "UTF8", (proErr, proContent) => {
						// If there was an issue reading the profile configuration file report it to the logs.
						if(proErr) {
							log.error("There was an issue in reading the profile configuration file located at " + profilePath + ".");
							errHandle(proErr);
						}
						// Otherwise proceed to assertain if a password is present.
						else {
							const curPass = JSON.parse(proContent).pass;
							// If no password is set load the application.
							if(curPass == "") { passProcess(splashWindow); }
							// Otherwise ask the user for a password to continue loading the application.
							else {
								splashWindow.webContents.send("passwordRequest");
								ipc.on("passwordCheck", (eveRes, val) => {
									let checker = curPass == crypto.createHash("sha256", val).update(val).digest("hex");
									eveRes.sender.send("passwordCheckResponse", checker);
									if(checker == true) {
										setTimeout(() => { passProcess(splashWindow); }, 1000);
									}
								});
							}
						}
