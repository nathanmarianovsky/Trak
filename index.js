		...
		fs.mkdir(assetsDir, { "recursive": true }, assetsDirErr => {
			...
			if(assetsDirErr) {
				...
				if(!fs.existsSync(path.join(basePath, "Trak", "config", "profile.json"))) {
					fs.writeFileSync(path.join(basePath, "Trak", "config", "profile.json"), JSON.stringify({"pass": ""}), "UTF8");
					passProcess(splashWindow);
				}
				else {
					fs.readFile(path.join(basePath, "Trak", "config", "profile.json"), "UTF8", (proErr, proContent) => {
						if(proErr) {
